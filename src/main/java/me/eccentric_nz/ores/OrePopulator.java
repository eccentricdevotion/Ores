...
public class OrePopulator extends BlockPopulator {
    ...
    public OrePopulator() {
        ...
        ores.add(new OreGenerator(Ore.ALUMINIUM, 17, 20, 128));
        ores.add(new OreGenerator(Ore.URANIUM, 9, 20, 64));
        ores.add(new OreGenerator(Ore.LEAD, 9, 20, 64));
    }

    @Override
    public void populate(WorldInfo worldInfo, Random random, int x, int z, LimitedRegion limitedRegion) {
        for (OreGenerator ore : ores) {
            ore.generate(limitedRegion, random);
        }
    }
}
